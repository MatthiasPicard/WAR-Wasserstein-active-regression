...
from ALWGAN_new_v3.dataset_handler import myData,WA_myData
...
class WAAL:
    ...
    def train(self,show_losses=False):
        ...
        trainset_labelled=WA_myData(self.X_train[idx_lb_train],self.y_train[idx_lb_train])
        trainset_unlabelled=WA_myData(self.X_train[idx_ulb_train],self.y_train[idx_ulb_train])
        ...
        trainloader_labelled= DataLoader(trainset_labelled,shuffle=True,batch_size=batch_labelled,drop_last=True)
        ...
        for epoch in range(self.total_epoch_train):  
            iterator_unlabelled=itertools.cycle(trainloader_unlabelled)
            iterator_labelled=itertools.cycle(trainloader_labelled)
            for i in range(nb_batch):
                label_x,label_y = next(iterator_labelled)
                unlabel_x,unlabel_y = next(iterator_unlabelled)
                #print(len(label_x.unique()))
                #print(len(label_x),len(label_y), len(unlabel_x),len(unlabel_y))
                #print(label_x, label_y, unlabel_x,unlabel_y)
    ...
    def Max_cost_B(self,idx_Xk,distance,i):
        ...
        #print(max(self.abs_cost_func(est_h_unl_X,bound_min),self.abs_cost_func(est_h_unl_X,bound_max)).detach().numpy()[0])
    ...
    def query(self):
        ...
        total_scores = losses - phi_scores.detach().numpy()    # removed csts
