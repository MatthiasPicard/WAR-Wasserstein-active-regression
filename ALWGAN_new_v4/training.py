...
from ALWGAN_new_v3.dataset_handler import myData
...
class WAAL:
    ...
    def train(self,show_losses,early_stopper,val_proportion):
        ...
        for epoch in range(self.total_epoch_phi):  
            ...
            for i in range(nb_batch):
                ...
                phi_ascent = self.phi(unlabel_x).mean() - cnst_T2 * self.phi(label_x).mean()  # removed abs  
                #print(phi_ascent.detach(),self.phi(unlabel_x).mean().detach(),cnst_T2 * self.phi(label_x).mean().detach(),self.phi(label_x).mean().detach(),cnst_T2)
    ...
    def query(self):
        ...
        total_scores = - phi_scores.detach().numpy() #+losses     # removed csts
