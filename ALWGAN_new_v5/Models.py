...
class NN_h(nn.Module):
    def __init__(self,dim_input,dim_output,norm,max_norm=1):
        
        dim_hidden= 64
        super(NN_h, self).__init__()
        self.linear1=direct_norm(torch.nn.Linear(dim_input,dim_hidden),kind="two-inf",max_norm=max_norm)
        self.group1=GroupSort(dim_hidden//2)
        self.linear2=direct_norm(torch.nn.Linear(dim_hidden,dim_hidden),kind=norm,max_norm=max_norm)
        self.linear3=direct_norm(torch.nn.Linear(dim_hidden,dim_hidden),kind=norm,max_norm=max_norm)
        self.linear4=direct_norm(torch.nn.Linear(dim_hidden,dim_hidden),kind=norm,max_norm=max_norm)
        self.linear5=direct_norm(torch.nn.Linear(dim_hidden,dim_hidden),kind=norm,max_norm=max_norm)
        self.linear6=direct_norm(torch.nn.Linear(dim_hidden,dim_hidden),kind=norm,max_norm=max_norm)
        self.linear7=direct_norm(torch.nn.Linear(dim_hidden,1),kind=norm,max_norm=max_norm)
